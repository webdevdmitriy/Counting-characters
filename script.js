...
function countChar() {
  ...
  let numCharWithoutSpaces = text.split(' ').join('').split('\n').join('').trim().length
...
btnShow.addEventListener('click', function () {
  textResult.style.display = textResult.style.display == 'block' ? 'none' : 'block'

  this.innerHTML = textResult.style.display == 'block' ? 'Скрыть получившийся текст' : 'Показать получившийся текст'
})
